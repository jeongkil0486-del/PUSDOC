/* =========================================================
   브리핑일지 모듈 (v3 - 2026-07 버그 수정판)
   - briefings/{YYYY-MM-DD} : 관리자가 달력에서 날짜를 클릭해 등록/수정/삭제
   ...
   - 직원/관리자 모두 동일한 '월 달력' UI를 사용하되, 클릭 시 동작만 다름
     (직원: 확인+서명 / 관리자: 등록+수정+삭제)

   v3 변경 사항
   - 이 파일에 남아있던 3세대에 걸친 중복 함수 정의를 제거하고 함수당 1개로 통일
     (이전에는 동일 함수가 2~3번 재정의되어 있었고, JS 함수 선언은 마지막 정의가
      이전 정의를 덮어쓰기 때문에 항상 파일 맨 마지막 정의만 실제로 동작했음.
      혼란을 막기 위해 실제로 쓰이던 최신 로직만 남기고 나머지는 삭제함)
   - cloneWorksheetFull: 병합 영역 안에서 서로 다른 셀이 갖고 있던 개별 테두리가
     mergeCells() 호출 시 마스터 셀 스타일로 전부 덮어써지던 문제를 수정
     (mergeCellsWithoutStyle 사용). 컬럼/행 스타일, 시트 보기(틀고정 등)도 복제.
   - insertSignatureImageCentered: 이미지를 시트 위에 고정 크기로 "붕 뜨게" 배치하던
     방식에서, 병합 셀 영역을 tl/br 앵커로 지정하는 방식으로 변경하여 실제 엑셀의
     "셀에 맞춰 이동 및 크기 조정" 옵션과 동일하게 동작하도록 함.
   - 관리자 브리핑 등록/수정 팝업에 [삭제] 버튼 추가 (deleteAdminBriefing)
========================================================= */
  ...
  modal.innerHTML = `
    <div class="modal-box" style="width:600px; max-width:94vw; max-height:90vh; overflow-y:auto;">
      ...
      <div class="modal-actions">
        ...
        <button class="modal-danger" id="adminBriefingDeleteBtn" onclick="deleteAdminBriefing()" style="flex:0 0 auto; padding:10px 16px; border:none; border-radius:8px; font-size:14px; cursor:pointer; background:#e74c3c; color:#fff;">삭제</button>
...
/* 브리핑일지 삭제 (관리자 전용)
   - briefings/{dateKey} 와 briefingConfirmations/{dateKey} 를 함께 삭제한다.
   - 확인 기록도 함께 지워야, 같은 날짜에 새 브리핑을 다시 등록했을 때
     직원들이 이전 서명이 남아있는 상태(=확인 완료)로 착각하지 않고
     다시 "확인 필요" 상태로 보이게 된다. */
async function deleteAdminBriefing() {
  const dateKey = activeAdminBriefingDateKey;
  const catId = activeAdminBriefingCatId;
  if (!dateKey) return;
  const existing = briefingsCache[dateKey] || null;
  if (!existing) { alert('삭제할 브리핑일지가 없습니다.'); return; }
  if (!confirm(`${dateKey} 브리핑일지를 삭제하시겠습니까?\n직원들의 확인/서명 기록도 함께 삭제되며, 이 작업은 되돌릴 수 없습니다.`)) return;
...
/* 원본 시트를 새 시트로 완전 복제한다.
   - 컬럼 너비/숨김/윤곽수준/스타일, 행 높이/숨김/윤곽수준, 셀 값과 스타일,
     시트 보기(틀고정 등)와 페이지 설정, 삽입 이미지, 병합 정보까지 복제한다.
   - 병합은 반드시 "모든 셀의 값/스타일을 먼저 채운 뒤 마지막에" 적용해야 하며,
     이때 ExcelJS의 mergeCells()가 아니라 mergeCellsWithoutStyle()을 사용해야 한다.
     mergeCells()는 병합 영역 안의 마스터(좌측 상단) 셀을 제외한 나머지 셀의
     스타일을 전부 마스터 셀 스타일로 덮어써 버리는데, 원본 양식은 병합 박스의
     안쪽/바깥쪽 테두리를 셀마다 다르게 설정해 그려 넣는 경우가 많아 이 과정에서
     테두리 선 일부가 사라지는 문제가 있었다. mergeCellsWithoutStyle()은 각 셀이
     원래 갖고 있던 스타일(테두리 포함)을 그대로 유지한 채 병합만 적용해 준다. */
function cloneWorksheetFull(workbook, sourceWs, newName) {
  const newWs = workbook.addWorksheet(newName, {
    properties: deepClonePlain(sourceWs.properties || {}),
    pageSetup: deepClonePlain(sourceWs.pageSetup || {}),
    views: sourceWs.views ? deepClonePlain(sourceWs.views) : undefined,
    headerFooter: sourceWs.headerFooter ? deepClonePlain(sourceWs.headerFooter) : undefined
  });

  const maxCol = Math.max(sourceWs.columnCount || 0, sourceWs.actualColumnCount || 0, 40);
  for (let c = 1; c <= maxCol; c++) {
    const srcCol = sourceWs.getColumn(c);
    if (!srcCol) continue;
    const dstCol = newWs.getColumn(c);
    if (srcCol.width != null) dstCol.width = srcCol.width;
    if (srcCol.hidden) dstCol.hidden = true;
    if (srcCol.outlineLevel) dstCol.outlineLevel = srcCol.outlineLevel;
    if (srcCol.style && Object.keys(srcCol.style).length) {
      try { dstCol.style = deepClonePlain(srcCol.style); } catch (e) { /* 컬럼 스타일 복사 실패 시 무시 */ }
    }
  }

  const maxRow = Math.max(sourceWs.rowCount || 0, sourceWs.actualRowCount || 0, 60);
  for (let r = 1; r <= maxRow; r++) {
    ...
    for (let c = 1; c <= colCount; c++) {
      ...
      if (srcCell.style) {
        try { dstCell.style = deepClonePlain(srcCell.style); } catch (e) { /* 스타일 복사 실패 시 무시 */ }
      }
    }
    if (dstRow.commit) dstRow.commit();
  }

  // 병합은 반드시 값/스타일을 다 채운 다음 마지막에 적용 (위 주석 참고)
  const merges = (sourceWs.model && sourceWs.model.merges) || [];
  merges.forEach(range => {
    try {
      if (typeof newWs.mergeCellsWithoutStyle === 'function') {
        newWs.mergeCellsWithoutStyle(range);
      } else {
        newWs.mergeCells(range);
      }
    } catch (e) { /* 이미 병합된 셀 등 무시 */ }
  });
...
async function insertSignatureImageCenteredInRange(workbook, worksheet, base64, mergeRange) {
  ...
  // 오프셋 픽셀을 행/열 단위 소수로 변환
  // (mergeRange.startCol/startRow는 1-based이고 ExcelJS tl.col/row는 0-based이므로 -1)
  const mergeColSpan = mergeRange.endCol - mergeRange.startCol + 1;
  const mergeRowSpan = mergeRange.endRow - mergeRange.startRow + 1;
  const tlCol = (mergeRange.startCol - 1) + (offsetX / box.width)  * mergeColSpan;
  const tlRow = (mergeRange.startRow - 1) + (offsetY / box.height) * mergeRowSpan;
